...
![サンプル入力で解析を完了した画面。入力カード、解析状態、独立したタイムラインカード（表示倍率つき）を表示](docs/images/ui-overview.png)

サンプル入力を使うと、ファイルを選ばずにローカル模擬解析を実行できます。画面右上の `LOCAL MOCK｜Gemini通信なし` が、外部通信をしないモードの目印です。タイムラインは動画カードとは別の全幅カードになっており、右下の「表示倍率」で拡大・縮小できます。

![動画表示、区間一覧・編集フォーム、JSON出力](docs/images/ui-editor.png)

区間一覧を見ながら、時刻、Job No.、標準書ページ、作業タイトルを修正できます。区間の色は分類ではなく番号の区別だけに使っています（選択中・現在位置・要確認は、タイムライン上で色と位置によって別々に示します）。
